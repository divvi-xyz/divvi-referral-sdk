...
describe.each([
	...
	it(`[${name}] should throw if the consumer or provider is not a valid address`, () => {
		...
		const goodAddress = '0x1234567890123456789012345678901234567890' as `0x${string}`;

		expect(() => getCallDataSuffix(goodAddress, [badAddress])).toThrow(InvalidAddressError);
		expect(() => getCallDataSuffix(badAddress, [goodAddress])).toThrow(InvalidAddressError);
		expect(() => getCallDataSuffix(badAddress, [badAddress])).toThrow(InvalidAddressError);
		expect(() => getCallDataSuffix(goodAddress, [goodAddress])).not.toThrow();
	});

	it(`[${name}] should throw if there is no consumer`, () => {
		const providers = ['0x0987654321098765432109876543210987654321' as `0x${string}`];

		expect(() => getCallDataSuffix('' as `0x${string}`, providers)).toThrow(InvalidAddressError);
	});

	it(`[${name}] should generate correct calldata suffix with no providers`, () => {
		const consumer = '0x1234567890123456789012345678901234567890' as `0x${string}`;
